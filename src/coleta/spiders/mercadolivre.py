    ...
    def parse(self, response):
        ...
        for product in products:
            ...
            yield {
                ...
                'name': products.css('h2.ui-search-item__title::text').get(),
                'old_price_reais': prices[0] if len(prices) > 0 else None,
                'old_price_centavos': cents[0] if len(cents) > 0 else None,
                'new_price_reais': prices[0] if len(prices) > 1 else None,
                'new_price_centavos': cents[0] if len(cents) > 1 else None,
